...
const steps = [
  {
    ...
    intro:
      "To can use image processing.You need to Login Or Signup first🥳.You can Login or Signup by clicking login button",
  },
];
const navSteps = [
  {
  ...
  },
  {
    ...
    intro:
      "In generative fill page you can resize your image with aspect ratio as you want.",
  },
  {
    element: "#ObjectRemove",
    intro:
      "In object remove page you can remove everything that you don't want, just by typing text you want to remove.🤩",
  },
  {
    element: "#ObjectRecolor",
    intro:
      "In object recolor page you can change color by typing color that you want to change.",
  },
  {
    element: "#BackgroundRemove",
    intro:
      "In background remove page you can remove your image background by clicking button.",
  },
  {
    element: "#Profile",
    intro:
      "In profile page, you can see your credits that will be used for every image processing. And you can also watch your images.",
  },
  {
    element: "#credit",
  ...
  },
  {
    element: "#BuyCredits",
    intro:
      "In credits page, you can buy credit that can use for image processing if you ran out your credits.",
  },
];
const Sidebar = () => {
  ...
  useEffect(() => {
    if (
      userId &&
      !localStorage.getItem("isDoneAuthNav" + userId) &&
      !user?.isAuthTourDone
    ) {
  ...
  const handleLoginStepExit = (stepIndex: any = steps.length) => {
    if (stepIndex !== 0) {
      localStorage.setItem("isDoneLoginStep", JSON.stringify(true));
    }
    setShowLoginStep(false);
  };
  const handleAuthNavStepExit = async (
    stepIndex: any = authNavSteps.length
  ) => {
    if (stepIndex != 0) {
      localStorage.setItem("isDoneAuthNav" + userId, JSON.stringify(true));
      const response = await updateTour(
        userId as string,
        !!localStorage.getItem("isDoneLoginStep"),
        !!localStorage.getItem("isDoneAuthNav" + userId)
      );
    }
          ...
          <nav className="sidebar-nav">
            ...
            <SignedOut>
              <Button
                asChild
                id="login-button"
                className="button bg-[#00273f] bg-cover"
              >
